...
void initStage() {
    ...
    SPR_initSprite(&sprites[0], &sprArle, 0, 0, TILE_ATTR(PAL1, TRUE, FALSE, FALSE));
    SPR_initSprite(&sprites[1], &sprCar, 0, 0, TILE_ATTR(PAL2, TRUE, FALSE, FALSE));
    ...
    //VDP_fillTileMapRect(VDP_PLAN_A, TILE_ATTR_FULL(PAL3, 0, 0, 0, TILE_USERINDEX), 0, 0, 40, 40);
    //VDP_fillTileMapRect(VDP_PLAN_A, TILE_ATTR_FULL(PAL3, 0, 0, 0, TILE_USERINDEX+1), 1, 12, 5, 4);
    u8 grassX, grassY;

    for(grassY = 0; grassY < 40; grassY += 2) {
        for(grassX = 0; grassX < 40; grassX += 2) {
            VDP_setTileMapXY(VDP_PLAN_A, TILE_ATTR_FULL(PAL3, 0, 0, 0, TILE_USERINDEX), grassX, grassY);
            VDP_setTileMapXY(VDP_PLAN_A, TILE_ATTR_FULL(PAL3, 0, 0, 0, TILE_USERINDEX+1), grassX+1, grassY);
            VDP_setTileMapXY(VDP_PLAN_A, TILE_ATTR_FULL(PAL3, 0, 0, 0, TILE_USERINDEX+2), grassX, grassY+1);
            VDP_setTileMapXY(VDP_PLAN_A, TILE_ATTR_FULL(PAL3, 0, 0, 0, TILE_USERINDEX+3), grassX+1, grassY+1);
        }
    }
...
static void updatePhysics() {
    if (joy1Input & BUTTON_UP) ytoward = -1;
    else if (joy1Input & BUTTON_DOWN) ytoward = 1;
    else ytoward = 0;

    if (joy1Input & BUTTON_LEFT) xtoward = -1;
    else if (joy1Input & BUTTON_RIGHT) xtoward = +1;
    else xtoward = 0;
    ...
    if (px_scr > FIX32(140)) npx_cam = entArle.posx - FIX32(140);
    else if (px_scr < FIX32(80)) npx_cam = entArle.posx - FIX32(80);
    else npx_cam = camposx;
    if (py_scr > FIX32(160)) npy_cam = entArle.posy - FIX32(160);
    else if (py_scr < FIX32(100)) npy_cam = entArle.posy - FIX32(100);
    ...
    SPR_setPosition(&sprites[0], fix32ToInt(entArle.posx), fix32ToInt(entArle.posy));
    SPR_setPosition(&sprites[1], fix32ToInt(entCar.posx), fix32ToInt(entCar.posy));
}
...
static void updateAnim() {
    ...
    if(entCar.hflip)
        SPR_setAttribut(&sprites[1], TILE_ATTR(PAL2, TRUE, FALSE, TRUE));
    else
        SPR_setAttribut(&sprites[1], TILE_ATTR(PAL2, TRUE, FALSE, FALSE));

    if(entArle.hflip)
        SPR_setAttribut(&sprites[0], TILE_ATTR(PAL1, TRUE, FALSE, TRUE));
    else
        SPR_setAttribut(&sprites[0], TILE_ATTR(PAL1, TRUE, FALSE, FALSE));
}
